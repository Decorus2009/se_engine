...
void req_sender::send_request(string const & req_str) {
...
//
    ...
    curl_easy_setopt(curl_, CURLOPT_WRITEDATA, &xml_content_stream);

//    CURLcode code = curl_easy_perform(curl_);
	curl_easy_perform(curl_);

	std::cout << "XML" << std::endl;
	std::cout << xml_content_stream.str() << std::endl;
//	xml_parser::parse(xml_content_stream);
}

req_sender::~req_sender() {

	std::cout << "CURL destr" << std::endl;
