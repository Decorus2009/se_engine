...
public:
    ...
    // FOR GTEST
    inline std::string const &get_preposition(size_t ind) const
    {
        return all_preps_.at(ind).prep_;
    }
    inline std::string const &get_request(size_t ind) const
    {
        return all_requests_.at(ind);
    }
